...
class Nudb {
  ...
  async search(query, timeout=10000) {
    let queryOptions = parseParams.getParamsQuery(query);
   ...
   * @param {String} options.recbeg Record begin pattern.
   ...
   * @param {Number} timeout Timeout, default is 10000 ms.
 */
  async deleteByQuery(query, timeout=10000) {
    let queryOptions = parseParams.getParamsQuery(query);
    queryOptions.db = parseParams.getParams(queryOptions.db || this.db);
    queryOptions.out = parseParams.getParamsOut(queryOptions.out);
    ...
    try {
      ...
      while(1) {
        const searchResult = await this.search(queryOptions, timeout);

        if(searchResult.result.cnt === 0) {
          break;
        }

        let deleteId = [];
        searchResult.result.recs.forEach((item) => {
          deleteId.push(item.rid);
        });
        
        const deleteResult = await this.rdel(deleteId.join(','), { timeout });
        deleteResult.result.forEach((item) => {
          if(item.error) {
            response.deleteCnt.fail++;
          } else if(item.result === 'OK'){
            response.deleteCnt.success++;
          }
        });
        response.result = response.result.concat(deleteResult.result);
      }
      return response;
    } catch(err) {
      throw new Error(err);
